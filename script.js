...
function initMap(mode) {
    ...
    // 3. Crear el mapa (esperamos 100ms para que el DOM se asiente)
    setTimeout(() => {
        map = L.map('map', { 
            zoomControl: false,
        ...
        // Forzar recalcular tamaño por si el CSS tardó
        map.invalidateSize();
...
function getFeatureId(feature) {
    if (currentMode === 'ar') {
        // En ar.json tus provincias están en NAME_1
        return feature.properties.NAME_1 ? feature.properties.NAME_1.toLowerCase() : "";
    } else {
        // En departamentos-tucuman.json están en departamen
...
function onEachFeature(feature, layer) {
    ...
    const info = regionData[dataKey][id] || { name: id, cap: "-" };
    ...
    layer.on('click', () => {
        ...
        // ACTUALIZACIÓN DE COLOR AL CLIQUEAR
...
function resetMap() {
    if(confirm("¿Seguro que quieres borrar todos los colores?")) {
        ...
        // REINICIO VISUAL: Aplicamos el estilo neutral a todas las capas
        if (geoJsonLayer) {
